...
export async function checkResponsive(options = {}) {
  ...
  for (const absolute of (await Promise.all(requested.map((directory) => findFiles(path.join(root, directory), (file) => file.endsWith(".html"))))).flat()) {
    ...
    completed.push(file);
    if (!/<meta[^>]+name=["']viewport["']/i.test(html)) findings.push(finding({ checker, rule: "responsive.viewport-missing", file, message: "Target does not declare a responsive viewport." }));
    if (/\bwidth\s*:\s*[1-9][0-9]{3,}px/i.test(html)) findings.push(finding({ checker, rule: "responsive.fixed-wide-width", file, message: "Target contains a fixed width likely to overflow narrow viewports." }));
  }
