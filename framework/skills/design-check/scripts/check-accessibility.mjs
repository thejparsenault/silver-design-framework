...
export async function checkAccessibility(options = {}) {
  ...
  for (const absolute of files) {
    ...
    for (const [pattern, rule, message] of rules) if (!pattern.test(html)) findings.push(finding({ checker, rule, file, message }));
    for (const match of html.matchAll(/<button\b([^>]*)>/gi)) if (!/\btype=/.test(match[1])) findings.push(finding({ checker, rule: "accessibility.button-type", file, message: "Buttons need an explicit type." }));
    for (const match of html.matchAll(/<img\b([^>]*)>/gi)) if (!/\balt=/.test(match[1])) findings.push(finding({ checker, rule: "accessibility.image-alt", file, message: "Images need an alt attribute." }));
    for (const match of html.matchAll(/<input\b([^>]*)>/gi)) {
      const id = match[1].match(/\bid=["']([^"']+)/)?.[1];
      if (id && !new RegExp(`<label[^>]+for=["']${id}["']`, "i").test(html)) findings.push(finding({ checker, rule: "accessibility.input-label", file, message: `Input ${id} needs an associated label.` }));
    }
  }
