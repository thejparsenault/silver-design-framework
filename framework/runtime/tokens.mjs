...
export async function writeResolvedTokenIndex({ tokensDir, outputPath }) {
...
export async function buildTokenCss({ tokensDir, outputPath, prefix = "ds" }) {
  registerOklchTransformGroup();
  const source = [`${tokensDir.replace(/\\/g, "/")}/**/*.tokens.json`];
  const buildPath = `${path.dirname(outputPath).replace(/\\/g, "/")}/`;
  ...
  const hexPath = path.join(path.dirname(outputPath), "tokens.hex.css");
  const oklchPath = path.join(path.dirname(outputPath), "tokens.oklch.css");
...
export async function buildDesignSystemTokens({
  ...
  const { content: jsonContent } = await writeResolvedTokenIndex({ tokensDir, outputPath: jsonOutput });
  await buildTokenCss({ tokensDir, outputPath: cssOutput, prefix });
