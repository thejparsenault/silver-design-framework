...
export async function readProbe(root, transportId) {
  try {
    ...
    return probe.transport === transportId ? probe : null;
  } catch {
    return null;
  }
}
...
export function applyProbe(provider, probe, now = new Date()) {
  ...
  if (!isFresh(probe, provider, now)) {
    const age = probeAge(probe, now);
    const timing = age < 0
      ? "Its timestamp is in the future, so it cannot be current evidence."
      : `It was recorded ${Math.round(age)} hours ago, past its freshness window.`;
    return {
      ...provider,
      availability_reason: `${provider.availability_reason} ${timing}`,
      probe_state: "stale",
    };
  }
...
export async function recordProbe({ root, probe, providers = [] }) {
  ...
  const relative = path.join(PROBE_DIR, `${probe.transport}.json`);
  await mutator.write(relative, `${JSON.stringify(probe, null, 2)}\n`);
  return { path: relative.split(path.sep).join("/"), probe };
}
